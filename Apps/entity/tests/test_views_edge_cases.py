...
class TestOrganizationViewsEdgeCases:
    ...
    def test_organization_create_missing_name(self, client, user):
        ...
        url = reverse('entity:organization_create')
        data = {
            'description': 'Test Description'
        }
    ...
    def test_organization_detail_not_found(self, client, user):
        ...
        client.force_login(user)
        url = reverse('entity:organization_detail', kwargs={'pk': 99999})
    ...
    def test_organization_update_inactive(self, client, user, organization):
        ...
        organization.save()
        client.force_login(user)
        url = reverse('entity:organization_update', kwargs={'pk': organization.pk})
        data = {
    ...
    def test_department_create_circular_reference(self, client, user, organization, department):
        ...
        url = reverse('entity:department_update', kwargs={'org_pk': organization.pk, 'pk': department.pk})
        data = {
    ...
    @patch('Apps.entity.views.team_create')
    def test_team_create_cross_department(self, mock_view, client, user, organization, department):
        ...
        url = reverse('entity:team_create', kwargs={'org_pk': organization.pk, 'dept_pk': department.pk})
        data = {
            'name': 'New Team',
            'description': 'Test Description',
    ...
    def test_department_delete_with_active_teams(self, client, user, organization, department, team):
        ...
        client.force_login(user)
        url = reverse('entity:department_delete', kwargs={'org_pk': organization.pk, 'pk': department.pk})
        response = client.post(url)
        assert response.status_code == 400

    @patch('Apps.entity.views.team_update')
    def test_team_update_invalid_parent(self, mock_view, client, user, organization, department, team):
        ...
        # Try to update team with parent from different department
        data = {
            'name': 'Updated Team',
            'department': department.id,
            'parent': other_team.id
        }
        response = client.post(
            reverse('entity:team_update', kwargs={
    ...
    def test_unauthorized_access(self, client, organization):
        ...
        urls = [
            reverse('entity:organization_list'),
            reverse('entity:organization_detail', kwargs={'pk': organization.pk}),
            reverse('entity:organization_create'),
            reverse('entity:organization_update', kwargs={'pk': organization.pk}),
            reverse('entity:organization_delete', kwargs={'pk': organization.pk})
        ]
        for url in urls:
            response = client.get(url)
